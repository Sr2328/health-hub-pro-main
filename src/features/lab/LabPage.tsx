...
import { generateLabReport } from "@/lib/pdf";
...
function StatCard({
  ...
  return (
    <Card
      className={`relative overflow-hidden transition-all cursor-pointer hover:shadow-md ${active ? "ring-2 ring-primary" : ""
        }`}
      onClick={onClick}
    >
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-lg">
        ...
        {/* Status Banner */}
        <div
          className={`rounded-lg p-3 border flex items-center gap-2 ${s.bg}`}
        >
          <span className={s.color}>{s.icon}</span>
          <span className={`font-semibold text-sm ${s.color}`}>{s.label}</span>
          {order.result_at && (
            <span className="ml-auto text-xs text-muted-foreground">
              Result at {format(parseISO(order.result_at), "MMM d, h:mm a")}
            </span>
          )}
        </div>
        ...
        {order.result && (
          <div className="rounded-lg border p-3 space-y-1">
            <p className="text-xs uppercase tracking-wide text-muted-foreground font-medium flex items-center gap-1">
              <ClipboardList className="h-3 w-3" /> Result
            </p>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {order.status !== "completed" && order.status !== "cancelled" && (
            ...
            >
              ...
              {order.result ? "Update Result" : "Enter Result"}
            </Button>
          )}
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-md">
        ...
        <Textarea
          ...
          placeholder="Enter test result, values, reference ranges…"
...
function NewOrderDialog({
  ...
  visits: any[];
  onSubmit: (data: { visit_id: string; test_name: string }) => void;
  isPending: boolean;
}) {
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState({ visit_id: "", test_name: "" });

  const handleSubmit = () => {
    onSubmit(form);
    if (!isPending) {
      setOpen(false);
      setForm({ visit_id: "", test_name: "" });
    }
  };
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-sm">
        <DialogHeader>
          ...
          <DialogDescription>
            Assign a test to an existing patient visit.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <Select
              ...
              onValueChange={(v) => setForm({ ...form, test_name: v })}
            >
              <SelectTrigger>
...
export function LabPage() {
      ...
      .on(
        ...
        (payload: any) => {
          ...
          if (
            ...
            toast.success(`Lab result ready: ${payload.new.test_name}`);
          } else if (payload.eventType === "INSERT") {
            toast.info(`New lab order: ${payload.new.test_name}`);
          }
        }
      )
      .subscribe();
  ...
  const create = useMutation({
    mutationFn: async (data: { visit_id: string; test_name: string }) => {
      if (!data.visit_id || !data.test_name)
        throw new Error("Visit and test are required");
      const { error } = await supabase
        .from("lab_orders")
        .insert({ visit_id: data.visit_id, test_name: data.test_name, status: "pending" });
  ...
  const submitResult = useMutation({
    ...
    onSuccess: () => {
      toast.success("Result saved and marked complete");
      qc.invalidateQueries({ queryKey: ["lab-orders"] });
      setResultOpen(false);
      setResultOrder(null);
    },
  ...
  const upload = async (id: string, file: File) => {
    ...
    const { data } = supabase.storage.from("lab-reports").getPublicUrl(path);
    const { error } = await supabase
      .from("lab_orders")
  ...
  return (
    <div className="space-y-5 pb-8">
      ...
      <PageHeader
        ...
        description="Test orders, results, and PDF reports — realtime"
        action={
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              size="sm"
              className="gap-1.5"
              onClick={() => generateLabReport(filtered)}
            >
                ...
                <TableRow>
                  <TableCell colSpan={7} className="text-center py-12">
                    <div className="flex flex-col items-center gap-2 text-muted-foreground">
                      ...
                      <span className="text-sm">Loading lab orders…</span>
                    </div>
                  </TableCell>
                </TableRow>
              ) : paginated.length === 0 ? (
                ...
                paginated.map((o: any) => (
                  ...
                  >
                    <TableCell className="pl-4">
                      <span className="font-medium text-sm">{o.test_name}</span>
                    </TableCell>
                      ...
                      <div className="flex items-center justify-end gap-1">
                        ...
                        {o.status !== "completed" && o.status !== "cancelled" && (
                          <Button
                            ...
                            title="Enter result"
                            onClick={(e) => { e.stopPropagation(); openResult(o); }}
                          >
                            <ClipboardList className="h-3.5 w-3.5" />
                          </Button>
                        )}
      ...
      <ResultDialog
        order={resultOrder}
        open={resultOpen}
        onClose={() => { setResultOpen(false); setResultOrder(null); }}
        onSubmit={(result) =>
          submitResult.mutate({ id: resultOrder.id, result })
        }
